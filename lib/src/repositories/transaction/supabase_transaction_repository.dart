...
class SupabaseTransactionRepository implements TransactionRepository {
    ...
    try {
      ...
      if (oldAccountId != null) {
        if (oldType == 'expense') {
          await _supabase.rpc(
            'increase_account_amount',
            params: {'p_account_id': oldAccountId, 'p_amount': oldAmount},
          );
      ...
      if (oldBudgetId != null) {
        if (oldType == 'expense') {
          await _supabase.rpc(
            'increase_budget_amount',
            params: {'p_budget_id': oldBudgetId, 'p_amount': oldAmount},
          );
        } else if (oldType == 'income') {
          await _supabase.rpc(
            'decrease_budget_amount',
            params: {'p_budget_id': oldBudgetId, 'p_amount': oldAmount},
          );
        }
      }
    ...
    try {
      ...
      if (accountId != null) {
        if (type == 'expense') {
          ...
          await _supabase.rpc(
            'increase_account_amount',
            params: {'p_account_id': accountId, 'p_amount': amount},
          );
        } else if (type == 'income') {
        ...
        if (type == 'expense') {
          ...
          await _supabase.rpc(
            'increase_budget_amount',
            params: {'p_budget_id': budgetId, 'p_amount': amount},
          );
        } else if (type == 'income') {
  ...
  // ============================================================
  @override
  Future<TransactionModel?> getPartnerTransfer(
    String id,
    String transferId,
  ) async {
  ...
  @override
  Future<Map<String, dynamic>> getAccountsBalances(String accountId) async {
    ...
    return {"balance": (result['balance'] as num).toDouble()};
  }

  @override
  Future<Map<String, double>> getAccountsBalances2(
    String fromId,
    String toId,
  ) async {
    final result = await _supabase
        .from('accounts')
        .select('id, balance')
        .inFilter('id', [fromId, toId]);

    return {
      for (final row in result) row['id']: (row['balance'] as num).toDouble(),
    };
  }
  @override
  Future<Map<String, dynamic>> getTransferPair(String transferId) async {
  final result = await _supabase
      .from('transactions')
      .select('*, accounts(name), categories(name)')
      .eq('transfer_id', transferId);
  ...
  final outJson = result.firstWhere((x) => x['type'] == 'transfer_out');
  final inJson = result.firstWhere((x) => x['type'] == 'transfer_in');

  final outTx = TransactionModel.fromJson(outJson);
  final inTx = TransactionModel.fromJson(inJson);

  final balances = await getAccountsBalances2(
    outTx.accountId!,
    inTx.accountId!,
  );
